function LILChecker(consoleElement, post) {
	...
	self.validateStructure = function() {
		...
		self.printLog('Images found: ' + found.length + '<br/>');
	...
	self.findImages = function() {
		...
		var found = false;
		self.tablesFound.forEach(function(image) {
				
			var img = image.url == NOT_FOUND ? '?' : '<img src="https://images.hive.blog/64x0/' + image.url.replace('https://images.hive.blog/0x0/', '') + '" width="64"/>';
			
			self.printLog('Title: ' + image.title + '<br/>Tags: ' + image.tags + '<br/>Image: ' + image.url + '<br/><div class="LMACLILChecker-console-image">' + img + '</div><br/><br/>');
			found = true;
		});
...
function checkLILPost() {
	checker = new LILChecker(
		document.getElementById('LMACLILChecker-console'), jQuery('#edit-hive-post').val()
	);
	...
	var workerInterval = setInterval(function () {
		...
		if (task == undefined || !task()) {
			...
			jQuery('#edit-submit-to-check').attr("disabled", false);
		}
	}, 1000);
}
...
jQuery(document).ready(function () { 
	jQuery('#edit-submit-to-check').click(function() {		
		jQuery('#edit-submit-to-check').attr("disabled", true);
		checkLILPost();		
		return false;
	});
